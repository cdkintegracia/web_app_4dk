...
# Смарт-процесс "Норматив ЭДО"
EDO_NORM_ENTITY_TYPE_ID = 1142
EDO_NORM_CATEGORY_ID = 142
EDO_NORM_FIELD_MONTH = 'ufCrm106_1786543555'
EDO_NORM_FIELD_YEAR = 'ufCrm106_1786543671'
EDO_NORM_FIELD_TRAFFIC = 'ufCrm106_1786543704'
EDO_NORM_FIELD_OK = 'ufCrm106_1786543718'
...
def get_edo_norm_enum_value_id(field_definition: dict, wanted_value: str) -> int:
    """Возвращает ID значения поля-списка смарт-процесса по его отображаемому значению."""
    for item in field_definition.get('items', []):
        item_value = str(item.get('VALUE', item.get('value', ''))).strip()
        if item_value == wanted_value:
            item_id = item.get('ID', item.get('id'))
            return int(item_id)

    available = [
        str(item.get('VALUE', item.get('value', ''))).strip()
        for item in field_definition.get('items', [])
    ]
    raise RuntimeError(
        f'В смарт-процессе Норматив ЭДО не найдено значение списка {wanted_value!r}. '
...
def get_edo_norm_items(report_month: int, report_year: int, month_name: str) -> list:
    ...
    if isinstance(fields_result, dict):
        fields = fields_result.get('fields') or {}
        if not fields and isinstance(fields_result.get('result'), dict):
            fields = fields_result['result'].get('fields') or {}

    if not fields:
        raise RuntimeError('Не удалось получить поля смарт-процесса Норматив ЭДО')

    month_field = fields.get(EDO_NORM_FIELD_MONTH)
    year_field = fields.get(EDO_NORM_FIELD_YEAR)
    if not month_field or not year_field:
        raise RuntimeError(
            'В смарт-процессе Норматив ЭДО не найдены поля Месяц/Год. '
            'Проверьте коды пользовательских полей.'
        )

    month_enum_id = get_edo_norm_enum_value_id(
        month_field,
        f'{report_month:02d} {month_name}'
    )
    year_enum_id = get_edo_norm_enum_value_id(year_field, str(report_year))

    return b.get_all('crm.item.list', {
        'entityTypeId': EDO_NORM_ENTITY_TYPE_ID,
        'select': [
            'id',
            'assignedById',
            EDO_NORM_FIELD_TRAFFIC,
            EDO_NORM_FIELD_OK,
        ],
        'filter': {
            'categoryId': EDO_NORM_CATEGORY_ID,
            EDO_NORM_FIELD_MONTH: month_enum_id,
            EDO_NORM_FIELD_YEAR: year_enum_id,
        }
    })


def bitrix_bool_is_true(value) -> bool:
    """Нормализует возможные варианты boolean, возвращаемые Битрикс24."""
    if value is True:
        return True
...
def create_employees_report(req):
    ...
    # Один раз получаем данные смарт-процесса за отчетный месяц,
    # затем внутри цикла только отбираем записи конкретного сотрудника.
    edo_norm_items_report_month = get_edo_norm_items(
        report_month,
        report_year,
        month_names[report_month]
    )
    ...
    for index, user_info in enumerate(users_info):
        ...
        before_before_last_month_deals_data = read_deals_data_file(before_before_last_month,
                                                                   before_before_last_month_year)
        ...
        its_prof_deals_last_month = list(filter(lambda x: x['Ответственный'] == user_name and x['Группа'] == 'ИТС' and
                                                          'Базовый' not in x['Тип'] and 'БизнесСтарт' not in x[
                                                              'Тип'] and 'ГРМ' not in x['Тип'] and
                                                          'Медицина' not in x['Тип'] and 'Индивидуальный' not in x[
                                                              'Тип'] and
                                                          x['Стадия сделки'] in ['Услуга активна', 'Счет сформирован',
                                                                                 'Счет отправлен клиенту'],
                                                last_month_deals_data))
        ...
        its_bizstart_deals_last_month = list(filter(lambda x: x['Ответственный'] == user_name and
                                                              ...
                                                              x['Стадия сделки'] in ['Услуга активна',
                                                                                     'Счет сформирован',
                                                                                     'Счет отправлен клиенту'],
                                                    last_month_deals_data))
        ...
        other_deals_last_month = list(filter(lambda x: x['Ответственный'] == user_name and
                                                       ...
                                                       x not in bitrix24_deals_last_month and x['Стадия сделки'] in [
                                                           'Услуга активна', 'Счет сформирован',
                                                           'Счет отправлен клиенту'], last_month_deals_data))

        # Предшествующий отчетному месяц
        its_prof_deals_before_last_month = list(
            filter(lambda x: x['Ответственный'] == user_name and x['Группа'] == 'ИТС' and
        ...
        its_base_deals_before_last_month = list(
            filter(lambda x: x['Ответственный'] == user_name and x['Группа'] == 'ИТС' and
                             ('Базовый' in x['Тип'] or 'Индивидуальный' in x['Тип']) and
                             x['Стадия сделки'] in ['Услуга активна', 'Счет сформирован', 'Счет отправлен клиенту'],
                   before_last_month_deals_data))
        ...
        its_bizstart_deals_before_last_month = list(filter(lambda x: x['Ответственный'] == user_name and
                                                                     x['Группа'] == 'ИТС' and 'БизнесСтарт' in x[
                                                                         'Тип'] and
                                                                     x['Стадия сделки'] in ['Услуга активна',
                                                                                            'Счет сформирован',
                                                                                            'Счет отправлен клиенту'],
        ...
        spark_in_contract_deals_before_last_month = list(
            filter(lambda x: x['Ответственный'] == user_name and x['Тип'] and
                             '1Спарк в договоре' == x['Тип'] and
                             x['Стадия сделки'] in ['Услуга активна', 'Счет сформирован', 'Счет отправлен клиенту'],
                   before_last_month_deals_data))

        spark_3000_deals_before_last_month = list(filter(lambda x: x['Ответственный'] == user_name and x['Тип'] and
                                                                   ('1Спарк 3000' == x['Тип'] or '1Спарк' == x[
                                                                       'Тип']) and
                                                                   x['Стадия сделки'] in ['Услуга активна',
                                                                                          'Счет сформирован',
                                                                                          'Счет отправлен клиенту'],
        ...
        spark_22500_deals_before_last_month = list(filter(lambda x: x['Ответственный'] == user_name and x['Тип'] and
                                                                    ('22500' in x['Тип'] or '1СпаркПЛЮС' in x[
                                                                        'Тип']) and
                                                                    x['Стадия сделки'] in ['Услуга активна',
                                                                                           'Счет сформирован',
                                                                                           'Счет отправлен клиенту'],
        ...
        bitrix24_deals_before_last_month = list(filter(lambda x: x['Ответственный'] == user_name and x['Тип'] and
                                                                 ('Битрикс24' == x['Тип'] or 'БИТРИКС24' == x[
                                                                     'Тип']) and
                                                                 x['Стадия сделки'] in ['Услуга активна',
                                                                                        'Счет сформирован',
                                                                                        'Счет отправлен клиенту'],
        ...
        other_deals_before_last_month = list(filter(lambda x: x['Ответственный'] == user_name and
                                                              ...
                                                              x not in bitrix24_deals_before_last_month and x[
                                                                  'Стадия сделки'] in ['Услуга активна',
                                                                                       'Счет сформирован',
                                                                                       'Счет отправлен клиенту'],
                                                    before_last_month_deals_data))
        ...
        other_deals_quarter = list(filter(lambda x: x['Ответственный'] == user_name and
                                                    ...
                                                    x not in bitrix24_deals_quarter and x['Стадия сделки'] in [
                                                        'Услуга активна', 'Счет сформирован', 'Счет отправлен клиенту'],
                                          quarter_deals_data))
        ...
        other_deals_start_year = list(filter(lambda x: x['Ответственный'] == user_name and
                                                       ...
                                                       x not in bitrix24_deals_start_year and x['Стадия сделки'] in [
                                                           'Услуга активна', 'Счет сформирован',
                                                           'Счет отправлен клиенту'], start_year_deals_data))
        ...
        if (len(bitrix24_deals_last_month) != 0) or (len(bitrix24_deals_before_last_month) != 0) or (
                len(bitrix24_deals_quarter) != 0) or (len(bitrix24_deals_start_year) != 0):
            worksheet.append([
                'Битрикс24',
        ...
        # Продление
        deals_ended_last_month_dpo = list(
            filter(lambda x: x['Дата проверки оплаты'] and x['Ответственный'] == user_name,
                   before_before_last_month_deals_data))
        deals_ended_last_month_dk = list(
            filter(lambda x: x['Ответственный'] == user_name, before_before_last_month_deals_data))
        deals_ended_last_month_dpo = list(map(lambda x: {'ID': x['ID'], 'Дата проверки оплаты': datetime.strptime(
            x['Дата проверки оплаты'], '%d.%m.%Y %H:%M:%S'), 'Предполагаемая дата закрытия': datetime.strptime(
            x['Предполагаемая дата закрытия'], '%d.%m.%Y'), 'Группа': x['Группа'], 'Регномер': x['Регномер'],
                                                         'Компания': x['Компания'], 'Тип': x['Тип']},
                                              deals_ended_last_month_dpo))
        deals_ended_last_month_dk = list(map(lambda x: {'ID': x['ID'],
                                                        'Предполагаемая дата закрытия': datetime.strptime(
                                                            x['Предполагаемая дата закрытия'], '%d.%m.%Y'),
                                                        'Дата проверки оплаты': '', 'Группа': x['Группа'],
                                                        'Регномер': x['Регномер'], 'Компания': x['Компания'],
                                                        'Тип': x['Тип']}, deals_ended_last_month_dk))
        deals_ended_last_month_dpo = list(filter(
            lambda x: datetime(day=1, month=before_last_month, year=before_last_month_year) <= x[
                'Дата проверки оплаты'] <= datetime(day=before_last_month_range_days, month=before_last_month,
                                                    year=before_last_month_year, hour=3), deals_ended_last_month_dpo))
        deals_ended_last_month_dpo_id = list(map(lambda x: x['ID'], deals_ended_last_month_dpo))
        deals_ended_last_month_dk = list(filter(lambda x: x['ID'] not in deals_ended_last_month_dpo_id and (
                    datetime(day=1, month=before_last_month, year=before_last_month_year) <= x[
                'Предполагаемая дата закрытия'] <= datetime(day=before_last_month_range_days, month=before_last_month,
                                                            year=before_last_month_year)), deals_ended_last_month_dk))
        deals_ended_last_month = deals_ended_last_month_dk + deals_ended_last_month_dpo

        last_month_deals_data_datetime_dpo = list(
            filter(lambda x: x['Ответственный'] == user_name and x['Дата проверки оплаты'], last_month_deals_data))
        last_month_deals_data_datetime_dpo = list(map(lambda x: {'ID': x['ID'],
                                                                 'Дата проверки оплаты': datetime.strptime(
                                                                     x['Дата проверки оплаты'], '%d.%m.%Y %H:%M:%S'),
                                                                 'Предполагаемая дата закрытия': '',
                                                                 'Стадия': x['Стадия сделки'],
                                                                 'Регномер': x['Регномер'], 'Компания': x['Компания'],
                                                                 'Тип': x['Тип']}, last_month_deals_data_datetime_dpo))
        last_month_deals_data_datetime_dk = list(
            filter(lambda x: x['Ответственный'] == user_name, last_month_deals_data))
        last_month_deals_data_datetime_dk = list(map(lambda x: {'ID': x['ID'],
                                                                'Предполагаемая дата закрытия': datetime.strptime(
                                                                    x['Предполагаемая дата закрытия'], '%d.%m.%Y'),
                                                                'Дата проверки оплаты': '',
                                                                'Стадия': x['Стадия сделки'], 'Регномер': x['Регномер'],
                                                                'Компания': x['Компания'], 'Тип': x['Тип']},
                                                     last_month_deals_data_datetime_dk))
        ...
        for deal in deals_ended_last_month:
            ...
            if not last_month_deal:
                ...
                if not last_month_deal[0]['Дата проверки оплаты'] or not deal['Дата проверки оплаты']:
                    continue
                if last_month_deal[0]['Дата проверки оплаты'] <= deal['Дата проверки оплаты'] and last_month_deal[0][
                    'Стадия'] != 'Услуга завершена':
                    non_extended_date_deals.append(deal)
        ...
        for deal in deals_ended_last_month:
            ...
            if not last_month_deal:
                ...
                if not last_month_deal[0]['Предполагаемая дата закрытия']:
                    continue
                if last_month_deal[0]['Предполагаемая дата закрытия'] <= deal['Предполагаемая дата закрытия'] and \
                        last_month_deal[0]['Стадия'] != 'Услуга завершена':
                    non_extended_date_deals.append(deal)

        ended_its = set(map(lambda x: x['ID'], list(filter(lambda x: x['Группа'] == 'ИТС', deals_ended_last_month))))
        ended_reporting = set(
            map(lambda x: x['ID'], list(filter(lambda x: x['Группа'] == 'Сервисы ИТС', deals_ended_last_month))))
        ended_others = set(map(lambda x: x['ID'], list(
            filter(lambda x: x['Группа'] not in ['Сервисы ИТС', 'ИТС'], deals_ended_last_month))))
        ...
        # Охват сервисами
        # Отчетный месяц
        companies = set(map(lambda x: x['Компания'],
                            list(filter(lambda x: x['Ответственный за компанию'] == user_name, last_month_deals_data))))
        companies_without_services_last_month = 0
        companies_without_paid_services_last_month = 0
        for company in companies:
            company_regnumbers = set(
                map(lambda x: x['Регномер'], list(filter(lambda x: x['Компания'] == company, last_month_deals_data))))
            company_its = list(
                filter(lambda x: x['Группа'] == 'ИТС' and company == x['Компания'], last_month_deals_data))
            if not company_its:
                continue

            non_prof_its = list(filter(lambda x: x['Группа'] == 'ИТС' and company == x['Компания'] and 'ПРОФ' not in x[
                'Тип'] and 'Облако' not in x['Тип'] and 'ГРМ' not in x['Тип'], last_month_deals_data))
            if non_prof_its:
                company_its_services = list(
                    filter(lambda x: (company == x['Компания'] or x['Регномер'] in company_regnumbers) and
                           ...
                           last_month_deals_data))  # если есть сделки итс компании КРОМЕ профов, то считаем все сервисы
        ...
        for company in companies:
            company_regnumbers = set(map(lambda x: x['Регномер'],
                                         list(
                                             filter(lambda x: x['Компания'] == company, before_last_month_deals_data))))
            company_its = list(
        ...
        try:
            coverage_its_without_paid_services_before_last_month = round(
                round(companies_without_paid_services_before_last_month /
                      len(its_deals_before_last_month), 2) * 100, 2)
        except ZeroDivisionError:
        ...
        try:
            coverage_free_reporting_deals_last_month = round(len(free_reporting_deals_last_month) /
                                                             (len(its_prof_deals_last_month) + len(
                                                                 its_bizstart_deals_last_month)), 2) * 100
        except ZeroDivisionError:
        ...
        for its_deal in its_deals_last_month:
            its_paid_reporting = list(filter(lambda x: (
                        x['Регномер'] == its_deal['Регномер'] and x['Тип'] == 'Отчетность' and x['Стадия сделки'] in [
                    'Услуга активна', 'Счет сформирован', 'Счет отправлен клиенту']), last_month_deals_data))
        ...
        for its_deal in its_deals_last_month:
            any_reporting = list(filter(lambda x: (
                        x['Регномер'] == its_deal['Регномер'] and 'Отчетность' in x['Тип'] and x['Стадия сделки'] in [
                    'Услуга активна', 'Счет сформирован', 'Счет отправлен клиенту']), last_month_deals_data))
            if any_reporting:
        ...
        try:
            coverage_free_reporting_deals_before_last_month = round(round(len(free_reporting_deals_before_last_month) /
                                                                          (len(its_prof_deals_before_last_month) + len(
                                                                              its_bizstart_deals_before_last_month)),
                                                                          2) * 100, 2)
        except ZeroDivisionError:
        ...
        for its_deal in its_deals_before_last_month:
            its_paid_reporting = list(
                filter(lambda x: (x['Регномер'] == its_deal['Регномер'] and x['Тип'] == 'Отчетность' and x[
                    'Стадия сделки'] in ['Услуга активна', 'Счет сформирован', 'Счет отправлен клиенту']),
                       before_last_month_deals_data))
            if its_paid_reporting:
        ...
        for its_deal in its_deals_before_last_month:
            any_reporting = list(filter(lambda x: (
                        x['Регномер'] == its_deal['Регномер'] and 'Отчетность' in x['Тип'] and x['Стадия сделки'] in [
                    'Услуга активна', 'Счет сформирован', 'Счет отправлен клиенту']), before_last_month_deals_data))
            if any_reporting:
        ...
        try:
            coverage_free_reporting_deals_start_year = round(round(len(free_reporting_deals_start_year) /
                                                                   (len(its_prof_deals_start_year) + len(
                                                                       its_bizstart_deals_start_year)), 2) * 100, 2)
        except ZeroDivisionError:
        ...
        for its_deal in its_deals_start_year:
            any_reporting = list(filter(lambda x: (
                        x['Регномер'] == its_deal['Регномер'] and 'Отчетность' in x['Тип'] and x['Стадия сделки'] in [
                    'Услуга активна', 'Счет сформирован', 'Счет отправлен клиенту']), start_year_deals_data))
            if any_reporting:
        ...
        worksheet.append(
            ['Апсейл', f'{month_names[report_month]} {report_year}', f'{month_names[before_last_month]} {report_year}'])
        ...
        if len(list_of_oldsales) > 1:
            ...
            for selling in list_of_oldsales:
                worksheet.append(
                    [selling['NAME_DEAL'], selling['COMPANY'], selling['OPPORTUNITY'], selling['DATE_SALE']])
            worksheet.append([])
        ...
        sum_provide_services = sum(
            list(map(lambda x: float(x['ufCrm41_1689101328'] if x['ufCrm41_1689101328'] else 0.0), provide_services)))

        if provide_services:
            list_provide_services = (
            [{'COMPANY': 'Компания', 'TYPE_PAY': 'Вид расчета', 'OPPORTUNITY': 'Сумма'}])  # 'TYPE_PAY': 'Вид расчета',
        ...
        sum_sold_services = sum(
            list(map(lambda x: float(x['ufCrm41_1689101328'] if x['ufCrm41_1689101328'] else 0.0), sold_services)))
        ...
        worksheet.append(
            ['Кол-во', len(tlp_tasks), len(worksits_tasks), len(pu_tasks), len(others_tasks), len(all_tasks)])
        worksheet.append(
            ['Трудозатраты', f'{tlp_hours} ч {tlp_minutes} мин', f'{worksits_hours} ч {worksits_minutes} мин',
             f'{pu_hours} ч {pu_minutes} мин', f'{others_hours} ч {others_minutes} мин',
             f'{all_hours} ч {all_minutes} мин'])
        ...
        edo_companies_id = list(
            map(lambda x: x['Компания'], list(filter(lambda y: 'Компания' in y and y['Компания'], all_its))))

        if edo_companies_id:
            edo_companies = b.get_all('crm.company.list', {
                'select': ['UF_CRM_1638093692254', 'UF_CRM_1638093750742'],
                'filter': {
                    'ID': list(map(lambda x: x['Компания'],
                                   list(filter(lambda y: 'Компания' in y and y['Компания'], all_its))))
                }
            })
            ...
            paid_traffic = list(filter(lambda x: int(list(x['PROPERTY_1573'].values())[0]) > 0 and int(
                list(x['PROPERTY_1575'].values())[0]) > 0, edo_elements_paid))
        ...
        # Выполнение норматива ЭДО
        employee_edo_norm_items = list(filter(
            lambda x: str(x.get('assignedById')) == str(user_info['ID']) and
                      int(x.get(EDO_NORM_FIELD_TRAFFIC) or 0) > 0,
            edo_norm_items_report_month
        ))

        edo_inn_with_traffic = len(employee_edo_norm_items)
        edo_inn_normative = len(list(filter(
            lambda x: bitrix_bool_is_true(x.get(EDO_NORM_FIELD_OK)),
            employee_edo_norm_items
        )))

        active_its_for_edo_norm = (
                len(its_prof_deals_last_month) +
                len(its_base_deals_last_month)
        )

        if active_its_for_edo_norm:
            edo_normative_rate = edo_inn_normative / active_its_for_edo_norm
        else:
            edo_normative_rate = 0

        worksheet.append([])
        worksheet.append(['Выполнение норматива ЭДО'])
        worksheet.append(['Месяц и год', f'{month_names[report_month]} {report_year}'])
        worksheet.append(['ИНН с трафиком', edo_inn_with_traffic])
        worksheet.append(['В том числе выполняющих норматив', edo_inn_normative])
        worksheet.append(['Всего действующих ИТС на конец месяца', active_its_for_edo_norm])
        worksheet.append(['Выполнение норматива', edo_normative_rate])
