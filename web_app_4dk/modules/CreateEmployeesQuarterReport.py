...
def create_employees_quarter_report(req):
    ...
    for index, user_info in enumerate(users_info):
        ...
        deals_ended_before_2_month_dk = list(map(lambda x: {'ID': x['ID'], 'Предполагаемая дата закрытия': datetime.strptime(x['Предполагаемая дата закрытия'], '%d.%m.%Y'), 'Дата проверки оплаты': '', 'Группа': x['Группа'], 'Регномер': x['Регномер'], 'Компания': x['Компания'], 'Тип': x['Тип']}, deals_ended_before_1_month_dk))
        deals_ended_before_2_month_dpo = list(filter(lambda x: datetime(day=1, month=before_1_month, year=before_1_month_year) <= x['Дата проверки оплаты'] <= datetime(day=before_1_month_range, month=before_1_month, year=before_1_month_year, hour=3), deals_ended_before_1_month_dpo))
