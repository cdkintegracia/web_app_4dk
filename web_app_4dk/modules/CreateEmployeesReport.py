                ...
                if deal_ids_old:
                    ...
                    for deal_last_month in old_sold_deals:
                        try:
                            ...
                            title = list(set(map(lambda x: x['TITLE'], list(filter(lambda x: x['ID'] == deal['Компания'], company_titles)))))
                            date_sale = list(filter(lambda x: x['parentId2'] is not None and int(x['parentId2']) == int(deal['ID']), oldsales))
                            date_sale = date_sale[0]['ufCrm3_1654248264'].strftime(ddmmyyyy_pattern)
                            print(date_sale)
                            if deal:
