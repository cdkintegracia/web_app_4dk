...
service_deal_values = {'Контрагент': 5800, 'Кабинет сотрудника': None, 'Линк': None, 'МДЛП': None, '1Спарк 3000': 3600,
                       '1СпаркПЛЮС 22500': 25500, 'Старт ЭДО': 3000, 'Подпись': 0, 'Подпись 1000': 1000, 'РПД': None,
                       'ЭТП': None, '1СПАРК Риски': 3600, '1Спарк в договоре': 3600, 'Кабинет садовода': 1000,
                       '1Спарк': 3600, 'ЭДО': None, 'mag1c': 5100, '1С-Администратор': 17500, 'Спарк сумма': None, '1Спарк 3600': 3600,'1СпаркПЛЮС 25500': 25500,'1СпаркПЛЮС': 25500}
...
def write_data_to_xlsx(data, month_titles=None, service_titles=None, month_count=None, update=False):
    ...
    worksheet.append([''])
    worksheet.append([''] + month_names + ['12 месяцев'])
    for month in month_names:
        for employee in data:
        ...
        for employee in service_report:
            if service_report[employee]['Подразделение'] == department:
                ...
                if services_month_summary:
                    worksheet.append([''])
                    worksheet.append(['' for _ in range(18)] + list(services_month_summary.values()))
                    services_month_summary = None
